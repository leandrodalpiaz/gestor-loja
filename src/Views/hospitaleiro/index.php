...
$appShellDescription = 'OcorrÃªncias assistenciais, visitas, retornos e encaminhamentos.';
...
<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 mb-8">
    ...
    <div class="card-metric"><p class="card-metric-label">ConcluÃ­das</p><p class="card-metric-value"><?= (int) ($resumo['concluidas'] ?? 0) ?></p></div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 space-y-8">
        <div class="card">
            <div class="card-header"><h2 class="card-title">PendÃªncias de Visita e Retorno</h2><p class="card-description">OcorrÃªncias que pedem presenÃ§a em campo ou acompanhamento ativo.</p></div>
            <div class="card-body space-y-4">
                <?php if (empty($pendenciasVisita)): ?>
                    <p class="text-center text-gray-500 py-4">Nenhuma pendÃªncia de visita no momento.</p>
                <?php else: ?>
                        ...
                        <div class="list-item-report">
                            ...
                            <?php if ($podeOperarOcorrencias): ?>
                                <form method="POST" action="/assistencia/ocorrencias/visita" class="mt-4 grid gap-3 md:grid-cols-[1fr_auto_auto]">
                                    ...
                                    <input type="text" name="observacao_visita" placeholder="ObservaÃ§Ã£o da visita" class="form-input">
        ...
        <div class="card">
            <div class="card-header"><h2 class="card-title">OcorrÃªncias Assistenciais Recentes</h2><p class="card-description">Fluxo assistencial de saÃºde, nascimento, falecimento, solidariedade e apoio geral.</p></div>
            <div class="card-body space-y-4">
                <?php foreach ($ocorrencias as $ocorrencia): ?>
                    <div class="list-item-report">
                        ...
                        <div class="mt-3 grid gap-2 text-xs text-gray-500 md:grid-cols-2">
                            ...
                            <span>Data da ocorrÃªncia: <strong><?= htmlspecialchars((string) ($ocorrencia['data_ocorrencia'] ?? '-')) ?></strong></span>
                            <span>PrÃ³xima aÃ§Ã£o: <strong><?= htmlspecialchars((string) ($ocorrencia['data_proxima_acao'] ?? '-')) ?></strong></span>
                        </div>
                            ...
                            <form method="POST" action="/assistencia/ocorrencias/status" class="mt-4 flex flex-wrap items-end gap-3">
                                ...
                                <div class="flex-grow">
                                    ...
                                    <select name="status" id="status-<?= $ocorrencia['id'] ?>" class="form-select w-full">
                                        ...
                                        <option value="concluida" <?= ($ocorrencia['status'] ?? '') === 'concluida' ? 'selected' : '' ?>>ConcluÃ­da</option>
                                        <option value="cancelada" <?= ($ocorrencia['status'] ?? '') === 'cancelada' ? 'selected' : '' ?>>Cancelada</option>
                                    </select>
                                </div>
                                <div class="flex-grow-[2]">
                                    <label for="obs-<?= $ocorrencia['id'] ?>" class="form-label sr-only">ObservaÃ§Ã£o</label>
                                    <input type="text" name="observacao_status" id="obs-<?= $ocorrencia['id'] ?>" placeholder="ObservaÃ§Ã£o de status" class="form-input w-full">
                                </div>
    ...
    <div class="space-y-8">
        <div class="card">
            <div class="card-header"><h2 class="card-title">Nova OcorrÃªncia Assistencial</h2><p class="card-description">Registro de ocorrÃªncia, visita, apoio e encaminhamento.</p></div>
            <div class="card-body">
                <?php if ($podeOperarOcorrencias): ?>
                    <form method="POST" action="/assistencia/ocorrencias/salvar" class="space-y-4">
                        <div><label for="tipo_ocorrencia" class="form-label">Tipo de ocorrÃªncia</label><select name="tipo_ocorrencia" id="tipo_ocorrencia" class="form-select"><option value="assistencia_geral">AssistÃªncia geral</option><option value="saude">SaÃºde</option><option value="nascimento">Nascimento</option><option value="falecimento">Falecimento</option><option value="solidariedade">Solidariedade</option></select></div>
                        <div><label for="obreiro_id" class="form-label">Obreiro vinculado</label><select name="obreiro_id" id="obreiro_id" class="form-select"><option value="">NÃ£o vincular obreiro</option><?php foreach ($obreiros as $obreiro): ?><option value="<?= htmlspecialchars((string) ($obreiro['id'] ?? '')) ?>"><?= htmlspecialchars((string) ($obreiro['nome_historico'] ?? $obreiro['nome'] ?? '')) ?> - CIM <?= htmlspecialchars((string) ($obreiro['cim'] ?? '-')) ?></option><?php endforeach; ?></select></div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div><label for="prioridade" class="form-label">Prioridade</label><select name="prioridade" id="prioridade" class="form-select"><option value="media">MÃ©dia</option><option value="baixa">Baixa</option><option value="alta">Alta</option><option value="urgente">Urgente</option></select></div>
                            <div><label for="encaminhar_para" class="form-label">Encaminhar para</label><select name="encaminhar_para" id="encaminhar_para" class="form-select"><option value="nenhum">Nenhum</option><option value="veneravel">VenerÃ¡vel Mestre</option><option value="tesoureiro">Tesoureiro</option><option value="ambos">VenerÃ¡vel + Tesoureiro</option></select></div>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div><label for="data_ocorrencia" class="form-label">Data da OcorrÃªncia</label><input type="date" name="data_ocorrencia" id="data_ocorrencia" class="form-input"></div>
                            <div><label for="data_proxima_acao" class="form-label">PrÃ³xima AÃ§Ã£o</label><input type="date" name="data_proxima_acao" id="data_proxima_acao" class="form-input"></div>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                        ...
                        <div><label for="descricao" class="form-label">DescriÃ§Ã£o detalhada</label><textarea name="descricao" id="descricao" rows="4" required class="form-textarea" placeholder="DescriÃ§Ã£o detalhada da ocorrÃªncia"></textarea></div>
                        <div class="space-y-2">
                        ...
                        <button type="submit" class="btn btn-primary w-full">Registrar OcorrÃªncia</button>
                    </form>
                <?php else: ?>
                    <p class="text-center text-gray-500 py-4">VocÃª nÃ£o tem permissÃ£o para registrar novas ocorrÃªncias.</p>
                <?php endif; ?>
            </div>
        </div>

        <div class="card">
            <div class="card-header"><h2 class="card-title">FunÃ§Ãµes do Cargo</h2></div>
            <div class="card-body">
