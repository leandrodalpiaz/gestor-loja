...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
    ...
    <div class="lg:col-span-2 space-y-8">
        ...
        <div class="card">
            <div class="card-header">
                <div><h2 class="card-title"><?= htmlspecialchars(trim((string) (($configuracaoLoja['nome_loja'] ?? '') . ' nÂº ' . ($configuracaoLoja['numero_loja'] ?? '')), " nÂº")) ?></h2><p class="card-description">Base institucional para relatórios e história da oficina.</p></div>
            ...
            <div class="card-body">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                    ...
                    <div class="list-item-param"><span>Reuniões</span><strong><?= htmlspecialchars(trim((string) (($configuracaoLoja['dia_semana_reuniao'] ?? '') . ' â€¢ ' . ($configuracaoLoja['horario_reuniao'] ?? '') . ' â€¢ ' . ($configuracaoLoja['periodicidade_reuniao'] ?? '')), ' â€¢')) ?></strong></div>
                </div>
        ...
        <div class="card-body">
            ...
            <form method="POST" action="/secretaria/sessoes/salvar" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div><label for="tipo_sessao_subtipo" class="form-label">Subtipo</label><select id="tipo_sessao_subtipo" name="tipo_sessao_subtipo" class="form-select"><?php foreach (['economica_1' => 'Econômica de 1Âº Grau', 'economica_2' => 'Econômica de 2Âº Grau', 'economica_3' => 'Econômica de 3Âº Grau', 'magna_iniciacao' => 'Magna de Iniciação', 'magna_elevacao' => 'Magna de Elevação', 'magna_exaltacao' => 'Magna de Exaltação', 'magna_instalacao' => 'Magna de Instalação', 'outra' => 'Outra'] as $v => $l): ?><option value="<?= $v ?>" <?= (($sessaoEmFormulario['tipo_sessao_subtipo'] ?? '') === $v) ? 'selected' : '' ?>><?= $l ?></option><?php endforeach; ?></select></div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-8">
        <div class="card"><div class="card-header"><h2 class="card-title">Trabalhos e Pe as de Arquitetura</h2><p class="card-description">Controle de trabalhos para a Ordem do Dia.</p></div><div class="card-body text-center text-gray-500"><p>Em breve: formul rio e lista de trabalhos refatorados.</p></div></div>

        <div class="card">
            <div class="card-header">
                <div>
                    <h2 class="card-title">Bala stre da Sessão em Foco</h2>
                    <p class="card-description">Rascunho pr -preenchido com Sessão, nominata e campos essenciais da ata.</p>
                </div>
            ...
            <div class="card-body">
                <?php if (empty($sessaoResumo) && empty($modoBalaustreIndependente)): ?>
                    <div class="space-y-4">
                        <div class="text-sm text-gray-600">Voc  pode preparar o bala stre manualmente em qualquer momento. Primeiro selecione a Sessão que ser  a base do rascunho.</div>
                        <form method="GET" action="/secretaria" class="grid grid-cols-1 md:grid-cols-3 gap-3">
                            <div class="md:col-span-2">
                                <label for="sessao_resumo_balaustre" class="form-label">Sessão para preparar o bala stre</label>
                ...
                <?php else: ?>
                    <form method="POST" action="/secretaria/balaustres/salvar" class="space-y-6">
                        ...
                        <div class="list-item-report">
                            <p class="font-semibold"><?= htmlspecialchars((string) ($sessaoResumo['titulo'] ?: (($sessaoResumo['tipo_sessao'] ?? 'Sessão') . ' - ' . ($sessaoResumo['grau_sessao'] ?? '')))) ?></p>
                            <p class="text-sm text-gray-500"><?= $formatDateTime((string) ($sessaoResumo['data_hora_inicio'] ?? '')) ?>   <?= (int) ($sessaoResumo['total_presentes'] ?? 0) ?> presen a(s) registradas</p>
                        </div>
                            ...
                            <div class="space-y-3 max-h-80 overflow-y-auto pr-1">
                                <?php foreach ($cargosBalaustreSessao as $cargoSessao): ?>
                                    <div class="grid grid-cols-1 md:grid-cols-4 gap-3 list-item-param !items-start">
                                        ...
                                        <div><label class="form-label">Observa  o</label><input name="cargo_sessao_observacao[]" value="<?= htmlspecialchars((string) ($cargoSessao['observacao'] ?? '')) ?>" class="form-input" placeholder="ad hoc, aus ncia..."></div>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                        </div>
                            ...
                            <div class="space-y-3">
                                <?php foreach ($linhasVisitantesBalaustre as $visitante): ?>
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3 list-item-param !items-start">
                                        ...
                                        <input name="palavra_visitante_potencia[]" value="<?= htmlspecialchars((string) ($visitante['potencia'] ?? '')) ?>" class="form-input" placeholder="Pot ncia">
                            ...
                            <div class="space-y-3">
                                <?php foreach ($linhasVisitasExternasBalaustre as $visitaExterna): ?>
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3 list-item-param !items-start">
                                        ...
                                        <input name="visita_externa_potencia[]" value="<?= htmlspecialchars((string) ($visitaExterna['potencia_obediencia'] ?? '')) ?>" class="form-input" placeholder="Pot ncia">
                                        <input type="date" name="visita_externa_data[]" value="<?= htmlspecialchars((string) ($visitaExterna['data_visita'] ?? '')) ?>" class="form-input">
                                        <input name="visita_externa_observacao[]" value="<?= htmlspecialchars((string) ($visitaExterna['observacao'] ?? '')) ?>" class="form-input" placeholder="Observa  o">
                                    </div>
                                <?php endforeach; ?>
                            </div>
                        </div>

                        <div><label for="texto_final" class="form-label">Texto final do bala stre (snapshot oficial)</label><textarea id="texto_final" name="texto_final" rows="5" class="form-textarea" readonly><?= htmlspecialchars((string) ($balaustreSessao['texto_final'] ?? '')) ?></textarea></div>
                        <div><label for="observacoes_secretaria" class="form-label">Observa  es da Secretaria</label><textarea id="observacoes_secretaria" name="observacoes_secretaria" rows="3" class="form-textarea"><?= htmlspecialchars($observacoesBalaustre) ?></textarea></div>
                        ...
                        <div class="flex flex-wrap gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <?php if (!empty($sessaoResumo['id']) && !empty($balaustreSessao['id']) && !in_array($balaustreSessao['status'] ?? '', ['apto_votacao', 'em_votacao', 'aprovado'], true)): ?>
                                <button type="submit" form="marcar-balaustre-apto" class="btn btn-warning">Marcar apto para vota  o</button>
                            <?php endif; ?>
                            <a href="/secretaria/votacao" class="btn btn-secondary">Acompanhar vota  es</a>
                        </div>
                    </form>
                    <?php if (!empty($sessaoResumo['id']) && !empty($balaustreSessao['id']) && !in_array($balaustreSessao['status'] ?? '', ['apto_votacao', 'em_votacao', 'aprovado'], true)): ?>
                        <form id="marcar-balaustre-apto" method="POST" action="/secretaria/balaustres/apto" onsubmit="return confirm('Marcar este bala stre como apto para vota  o?');">
                            <input type="hidden" name="balaustre_id" value="<?= (int) ($balaustreSessao['id'] ?? 0) ?>">
                        </form>
                    <?php endif; ?>
                <?php endif; ?>
