<?php
$erpPageTitle = 'SessÃ£o do Chanceler - EfemÃ©rides';
...
$appShellSidebarSections = [
    [
        'title' => 'Chancelaria',
        'items' => [
            ['label' => 'EfemÃ©rides', 'href' => '/chancelaria/efemerides'],
    ...
    <div class="space-y-6">
        ...
        <section class="rounded-3xl border border-erp-border bg-white px-6 py-5 shadow-sm">
            <div class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-end">
                <div>
                    <p class="text-xs font-semibold uppercase tracking-[0.24em] text-erp-gold">Acesso rÃ¡pido</p>
                    <h2 class="mt-2 text-2xl font-semibold text-erp-navy">Tarefas da Chancelaria</h2>
                    <p class="mt-2 text-sm text-erp-muted">Priorize mensagem do dia, correÃ§Ãµes de registro e envio oficial sem trocar de tela.</p>
                </div>
        ...
        <div class="hidden">
            <div class="mb-4">
                <p class="text-[11px] font-semibold uppercase tracking-[0.24em] text-slate-400">Acesso rÃ¡pido</p>
                <h2 class="mt-2 text-2xl font-semibold text-slate-900">Tarefas da Chancelaria</h2>
                <p class="mt-1 text-sm text-slate-700">Aqui vocÃª encontra os atalhos mais usados: certificado, mensagem do dia e atualizaÃ§Ã£o dos registros.</p>
            </div>
            <div class="mb-4 flex flex-wrap gap-2">
                ...
                <a href="/miniapp/data-maconica" class="px-3 py-2 text-sm rounded bg-slate-100 text-slate-700 hover:bg-slate-200">Datas maÃ§Ã´nicas</a>
                <a href="/miniapp/historico" class="px-3 py-2 text-sm rounded bg-slate-100 text-slate-700 hover:bg-slate-200">Fatos histÃ³ricos</a>
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <strong>Revisar mensagem:</strong> ajusta apenas o texto que serÃ¡ enviado hoje.
                <strong class="ml-2">Corrigir dados:</strong> ajusta nome, data, vÃ­nculo, local e demais registros no banco.
            </p>
        </div>
        ...
        <div class="grid grid-cols-1 gap-6 2xl:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)]">
            <section id="secao-mensagem" class="rounded-3xl border bg-white p-5 shadow-sm <?= $focoTela === 'mensagem' ? 'border-blue-400 ring-2 ring-blue-100' : 'border-slate-200' ?>">
                <div class="flex items-center justify-between mb-2">
                    ...
                    <span class="text-xs text-gray-500">Gerada automaticamente apÃ³s 00:01</span>
                </div>
                <p class="mb-3 text-sm text-gray-600">Esta ediÃ§Ã£o altera somente a mensagem de hoje. Os registros oficiais da base nÃ£o sÃ£o modificados aqui.</p>
                <?php
                ...
                <form method="POST" action="/chancelaria/efemerides/salvar-previa">
                    ...
                    <p class="mt-2 text-xs text-gray-500">MantÃ©m HTML do Telegram (ex.: <b> e <i>).</p>
                ...
                <div class="mt-3 flex flex-wrap gap-2">
                    <form method="POST" action="/chancelaria/efemerides/enviar-previa" onsubmit="return confirm('Enviar a prÃ©via para o Telegram privado do chanceler?');">
                        <button type="submit" class="px-3 py-2 text-sm rounded bg-indigo-700 text-white hover:bg-indigo-800">Enviar prÃ©via no privado</button>
                    </form>
                ...
                <p class="mt-2 text-xs text-gray-500">SugestÃ£o de uso: revisar, enviar no privado e, depois da conferÃªncia, publicar no grupo.</p>
            </section>
            ...
            <section id="secao-dados" class="rounded-3xl border bg-white p-5 shadow-sm <?= $focoTela === 'dados' ? 'border-emerald-400 ring-2 ring-emerald-100' : 'border-slate-200' ?>">
                <p class="text-[11px] font-semibold uppercase tracking-[0.24em] text-slate-400">Registro de efemÃ©rides</p>
                <h2 class="mt-2 text-xl font-semibold text-slate-900">Atualizar registros da Loja</h2>
                <p class="mb-3 text-sm text-slate-700">Os dados salvos aqui passam a valer tambÃ©m para os prÃ³ximos envios.</p>
                <form method="POST" action="/chancelaria/efemerides/salvar" class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div>
                        <label class="text-sm block mb-1">VÃ­nculo</label>
                        ...
                        <p class="text-xs text-gray-500 mt-1">Tratamento no texto Ã© automÃ¡tico (sobrinho, sobrinha, cunhada).</p>
                    </div>

                    <div>
                        <label class="text-sm block mb-1">Parentesco (irmÃ£o relacionado)</label>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="mensagem_custom" rows="4" class="w-full border border-gray-300 rounded px-3 py-2" placeholder="Para HistÃ³ria, informe o texto completo aqui."></textarea>
                    </div>
        ...
        <section class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <p class="mb-4 text-sm text-slate-700">Escolha um irmÃ£o para ver apenas os registros dele. Se preferir, vocÃª tambÃ©m pode abrir a lista completa.</p>
            ...
            <div class="mb-4 grid gap-3 md:grid-cols-[1.8fr_1.5fr_1fr_1fr_auto]">
                <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                    ...
                    <p class="mt-2 text-sm text-slate-700">Selecione um irmÃ£o para visualizar apenas os registros vinculados a ele.</p>
                </div>
                <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                    ...
                    <p class="mt-2 text-sm text-slate-700"><?= htmlspecialchars((string) (($filtroIrmaoRef ?? '') !== '' ? ($filtroIrmaoRef ?? '') : 'Todos os irmÃ£os')) ?></p>
                </div>
            ...
            <form method="GET" action="/chancelaria/efemerides" class="mb-5 grid grid-cols-1 gap-3 xl:grid-cols-[1.4fr_1.8fr_1fr_1fr_1fr_auto]">
                <div>
                    <label class="mb-1 block text-sm">Selecionar irmÃ£o</label>
                    <select name="irmao_ref" class="w-full border border-gray-300 rounded px-3 py-2">
                        <option value="">Todos os irmÃ£os</option>
                ...
                <div>
                    <label class="mb-1 block text-sm">Pesquisar por irmÃ£o, nome ou vÃ­nculo</label>
                    <input type="text" name="termo" value="<?= htmlspecialchars($filtroTermo ?? '') ?>" class="w-full border border-gray-300 rounded px-3 py-2" placeholder="Ex.: nome do irmÃ£o">
                </div>
                <div>
                ...
                <div>
                    <label class="mb-1 block text-sm">VÃ­nculo</label>
                    <select name="vinculo" class="w-full border border-gray-300 rounded px-3 py-2">
                        <option value="">Todos</option>
                        <?php foreach ($vinculosPadrao as $itemVinculo): ?>
                            <?php $nomeVinculo = trim((string) ($itemVinculo['nome'] ?? '')); ?>
                    ...
                    <article class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                ...
                                <div class="mt-1 text-sm text-slate-700"><?= htmlspecialchars((string) ($r['tipo'] ?? '')) ?> â€¢ <?= htmlspecialchars($formatarDataVisual((string) ($r['data_evento'] ?? ''))) ?></div>
                            </div>
                        ...
                        <div class="mt-3 text-sm text-slate-700">
                            <div>VÃ­nculo: <?= htmlspecialchars((string) ($r['vinculo'] ?? '-')) ?></div>
                            <div>Parentesco: <?= htmlspecialchars((string) ($r['parentesco'] ?? '-')) ?></div>
                        </div>
                        <?php if ($somenteLeitura): ?>
                            <div class="mt-4 text-sm text-slate-700">Registro fixo do sistema. Para alterar, edite o arquivo de histÃ³ricos.</div>
                        <?php elseif (!empty($r['ativo'])): ?>
                            <details class="mt-4 rounded-2xl border border-slate-200 bg-white p-3">
                                ...
                                <form method="POST" action="/chancelaria/efemerides/atualizar" class="mt-3 space-y-2">
                                    ...
                                    <input type="text" name="parentesco" value="<?= htmlspecialchars((string) ($r['parentesco'] ?? '')) ?>" class="w-full rounded border border-gray-300 px-3 py-2" placeholder="Nome do irmÃ£o relacionado">
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-slate-50 text-xs uppercase tracking-[0.16em] text-slate-500">
                        <tr>
                            ...
                            <th class="text-left px-3 py-3">VÃ­nculo/Parentesco</th>
                            <th class="text-left px-3 py-3">Status</th>
                            <th class="text-left px-3 py-3">AÃ§Ã£o</th>
                        </tr>
                    </thead>
                    <tbody>
                    <?php foreach (($registrosRecentes ?? []) as $r): ?>
                                ...
                                <?php elseif (!empty($r['ativo'])): ?>
                                    <details class="rounded-xl border border-gray-200 bg-gray-50 p-3">
                                        ...
                                        <form method="POST" action="/chancelaria/efemerides/atualizar" class="mt-3 space-y-2">
                                            ...
                                            <div>
                                                <label class="block text-xs text-gray-600 mb-1">VÃ­nculo</label>
                                            ...
                                            <div>
                                                <label class="block text-xs text-gray-600 mb-1">IrmÃ£o relacionado (parentesco)</label>
                                                <input type="text" name="parentesco" value="<?= htmlspecialchars((string) ($r['parentesco'] ?? '')) ?>" class="w-full border border-gray-300 rounded px-2 py-1" placeholder="Nome do irmÃ£o relacionado">
                                            </div>
                                            <div>
                                                <label class="block text-xs text-gray-600 mb-1">Local/Loja</label>
                                                <input type="text" name="local" value="<?= htmlspecialchars((string) ($r['local'] ?? '')) ?>" class="w-full border border-gray-300 rounded px-2 py-1" placeholder="Ex.: Loja RenascenÃ§a nÂº 270">
                                            </div>
                                            <div>
                                            ...
                                            <div class="flex gap-2">
                                                <button type="submit" class="flex-1 rounded bg-blue-700 px-3 py-2 text-sm text-white hover:bg-blue-800">Salvar ediÃ§Ã£o</button>
                                                <button type="submit" formaction="/chancelaria/efemerides/desativar" name="id" value="<?= (int) ($r['id'] ?? 0) ?>" class="flex-1 rounded bg-slate-200 px-3 py-2 text-sm text-slate-700 hover:bg-slate-300">Desativar</button>
                                            </div>
                                        </form>
                                    </details>
                                <?php else: ?>
                                    <span class="text-gray-500">Sem aÃ§Ãµes para inativos</span>
                                <?php endif; ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
