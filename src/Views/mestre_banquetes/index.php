...
<body class="erp-readable min-h-screen bg-[linear-gradient(180deg,#f8f4ea_0%,#eef2f7_100%)] font-sans text-slate-900">
    <div class="mx-auto max-w-6xl px-4 py-8">
        <header class="mb-8 rounded-3xl border border-white/40 bg-[radial-gradient(circle_at_top_left,#d3b269,transparent_28%),linear-gradient(135deg,#172030,#27364a)] px-6 py-7 text-white shadow-2xl">
            ...
            <h1 class="mt-2 text-3xl font-semibold">Controle do Ã¡gape e confirmados</h1>
            <p class="mt-2 max-w-2xl text-sm text-slate-200">O cargo agora trabalha por sessÃ£o em foco, com previsÃ£o de participantes, observaÃ§Ãµes logÃ­sticas e status operacional do banquete.</p>
        ...
        $dashboard = [
            ...
            'subtitle' => 'ExecuÃ§Ã£o operacional e registro de operaÃ§Ã£o do cargo.',
            'meta' => ['Perfil: operaÃ§Ã£o de sessÃ£o'],
            'actions' => [
                ['label' => 'Registrar operaÃ§Ã£o', 'href' => '/mestre-banquetes/operacao/salvar'],
            ],
            'blocks' => [
                ['title' => 'OperaÃ§Ãµes recentes', 'subtitle' => 'Status e previsÃ£o de participantes.', 'span' => 'half', 'metrics' => [
                    ['label' => 'Confirmados', 'value' => (string) count($confirmados)],
                    ['label' => 'Participantes do Ã¡gape', 'value' => (string) count($participantesAgape)],
                ], 'list' => [['item' => 'Status operacional', 'meta' => (string) ($operacaoBanquete['status_operacional'] ?? 'planejamento'), 'status' => 'Regular']]],
                ['title' => 'SessÃµes relacionadas e histÃ³rico', 'subtitle' => 'Leitura simples da agenda.', 'span' => 'half', 'metrics' => [
                    ['label' => 'SessÃµes na agenda', 'value' => (string) count($sessoes)],
                    ['label' => 'Sem Ã¡gape', 'value' => (string) count($semAgape)],
                ], 'list' => array_map(static fn (array $s): array => ['item' => (string) ($s['titulo'] ?: (($s['tipo_sessao'] ?? 'SessÃ£o') . ' - ' . ($s['grau_sessao'] ?? ''))), 'meta' => (string) ($s['data_hora_inicio'] ?? ''), 'status' => (string) ($s['status'] ?? '-')], array_slice($sessoes, 0, 4))],
            ],
            'alerts' => [['title' => 'OperaÃ§Ã£o de Ã¡gape', 'text' => 'Manter previsÃ£o e status alinhados com a sessÃ£o em foco.', 'tone' => 'warning']],
        ...
        <div class="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
            <section class="space-y-6">
                <article class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                        <div>
                            ...
                            <p class="mt-2 text-sm text-slate-700">O Mestre de Banquetes pode alternar a sessÃ£o de trabalho para ajustar previsÃ£o e operaÃ§Ã£o do Ã¡gape.</p>
                        </div>
                    ...
                    <?php if ($sessaoEmFoco): ?>
                        <div class="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            ...
                            <div class="mt-3 flex flex-wrap gap-2 text-xs">
                                ...
                                <span class="rounded-full bg-white px-3 py-1 text-slate-700">Ãgape: <?= count($participantesAgape) ?></span>
                            ...
                            <div class="mt-3 grid gap-2 text-xs text-slate-700">
                                <div>ConfiguraÃ§Ã£o do Ã¡gape: <?= htmlspecialchars((string) ($descricaoAgape ?? 'NÃ£o informado')) ?></div>
                                <div>Modelo financeiro: <?= htmlspecialchars((string) ($descricaoModeloTesourariaAgape ?? 'NÃ£o informado')) ?></div>
                            </div>
                        </div>
                    <?php else: ?>
                ...
                <article class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="flex items-center justify-between gap-3">
                        <div>
                            <h2 class="text-2xl font-semibold text-slate-900">OperaÃ§Ã£o do banquete</h2>
                            <p class="mt-2 text-sm text-slate-700">Registre previsÃ£o, observaÃ§Ãµes e o status logÃ­stico do Ã¡gape.</p>
                        </div>
                    ...
                    <form method="POST" action="/mestre-banquetes/operacao/salvar" class="mt-4 space-y-3">
                        ...
                        <div>
                            <label class="mb-1 block text-sm font-medium text-slate-700">PrevisÃ£o de participantes</label>
                        ...
                        <div>
                            <label class="mb-1 block text-sm font-medium text-slate-700">ObservaÃ§Ãµes logÃ­sticas</label>
                        ...
                        <button type="submit" class="rounded-xl bg-amber-600 px-4 py-3 text-sm font-semibold text-white hover:bg-amber-500">Salvar operaÃ§Ã£o</button>
                    </form>
                </article>

                <article class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    <h2 class="text-2xl font-semibold text-slate-900">Participantes do Ã¡gape</h2>
                    <p class="mt-2 text-sm text-slate-700">Lista prÃ¡tica dos confirmados que optaram por participar do Ã¡gape.</p>
                    <div class="mt-4 space-y-3">
                        <?php if ($participantesAgape !== []): ?>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">Ainda nÃ£o hÃ¡ participantes confirmados com Ã¡gape.</div>
                        <?php endif; ?>
                    </div>
                </article>
            </section>
            ...
            <aside class="space-y-6">
                <article class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    <h2 class="text-2xl font-semibold text-slate-900">Confirmados sem Ã¡gape</h2>
                    <div class="mt-4 space-y-3">
                        <?php if ($semAgape !== []): ?>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">NÃ£o hÃ¡ confirmados sem Ã¡gape neste momento.</div>
                        <?php endif; ?>
                    </div>
                </article>
                    ...
                    <div class="mt-4 space-y-3">
                        <?php foreach ($sessoes as $sessao): ?>
                            <a href="/mestre-banquetes?sessao_id=<?= (int) ($sessao['id'] ?? 0) ?>" class="block rounded-2xl border border-slate-200 bg-white px-4 py-3 hover:border-amber-300">
                                <div class="font-medium text-slate-900"><?= htmlspecialchars($sessao['titulo'] ?: (($sessao['tipo_sessao'] ?? 'SessÃ£o') . ' - ' . ($sessao['grau_sessao'] ?? ''))) ?></div>
                                <div class="mt-1 text-sm text-slate-700"><?= htmlspecialchars((string) ($sessao['data_hora_inicio'] ?? '')) ?></div>
                                <div class="mt-2 text-xs text-slate-700">Confirmados: <?= (int) ($sessao['total_confirmados'] ?? 0) ?> Â· Ãgape: <?= (int) ($sessao['total_agape'] ?? 0) ?></div>
                            </a>
                        <?php endforeach; ?>
                    </div>
                </article>
