...
$appShellTitle = 'Controle de Empr̩stimos';
$appShellDescription = 'Gerencie os empr̩stimos pendentes, atrasados e registre as devolṳ̵es.';
...
<div class="space-y-4 md:hidden">
    <?php if (empty($emprestimos)): ?>
        <div class="card-placeholder">Nenhum empr̩stimo pendente ou atrasado.</div>
    <?php else: ?>
                    ...
                    <?php if (($emp['status'] ?? '') === 'pendente' || ($emp['status'] ?? '') === 'atrasado'): ?>
                        <div class="mt-4">
                            <form action="/biblioteca/devolver" method="POST">
                                ...
                                <button type="submit" class="btn btn-primary w-full">Registrar Devolṳ̣o</button>
                            </form>
                        </div>
                    <?php endif; ?>
                </div>
        ...
        <table class="table-base">
            <thead>
                <tr>
                    ...
                    <th>Status</th>
                    <th class="w-48">A̤̣o</th>
                </tr>
            </thead>
            <tbody>
                <?php if (empty($emprestimos)): ?>
                    <tr>
                        <td colspan="6" class="text-center py-10 text-gray-500">Nenhum empr̩stimo pendente ou atrasado.</td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($emprestimos as $emp): ?>
                        <?php $statusInfo = getStatusInfo((string) ($emp['status'] ?? '-')); ?>
                        <tr>
                            ...
                            <td class="text-right">
                                <?php if (($emp['status'] ?? '') === 'pendente' || ($emp['status'] ?? '') === 'atrasado'): ?>
                                    <form action="/biblioteca/devolver" method="POST">
                                        <input type="hidden" name="id" value="<?= htmlspecialchars((string) ($emp['id'] ?? '')) ?>">
                                        <button type="submit" class="btn btn-primary">Registrar Devolṳ̣o</button>
                                    </form>
                                <?php else: ?>
