...
<head>
    ...
    <title>Tesouraria e SessÃµes - Gestor da Loja</title>
...
<body class="min-h-screen bg-[linear-gradient(180deg,#f8f4ea_0%,#eef2f7_100%)] font-sans text-slate-900">
    <div class="mx-auto max-w-6xl px-4 py-8">
        <header class="mb-8 rounded-3xl border border-white/40 bg-[radial-gradient(circle_at_top_left,#8fd0aa,transparent_28%),linear-gradient(135deg,#14324a,#1f4d63)] px-6 py-7 text-white shadow-2xl">
            <p class="text-xs uppercase tracking-[0.24em] text-emerald-200">Tesouraria e SessÃµes</p>
            <h1 class="mt-2 text-3xl font-semibold">Reflexo financeiro do Ã¡gape</h1>
            <p class="mt-2 max-w-3xl text-sm text-slate-200">A sessÃ£o abastece a Tesouraria com a leitura do Ã¡gape pago, a quantidade confirmada, o valor unitÃ¡rio e a estimativa de arrecadaÃ§Ã£o antes da realizaÃ§Ã£o.</p>
        ...
        <div class="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
            <section class="space-y-6">
                <article class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    <h2 class="text-2xl font-semibold text-slate-900">PrÃ³xima sessÃ£o com leitura financeira</h2>
                    <?php if ($proximaSessao): ?>
                        <div class="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            <div class="text-lg font-semibold text-slate-900"><?= htmlspecialchars($proximaSessao['titulo'] ?: (($proximaSessao['tipo_sessao'] ?? 'SessÃ£o') . ' - ' . ($proximaSessao['grau_sessao'] ?? ''))) ?></div>
                            ...
                            <div class="mt-4 grid gap-3 md:grid-cols-2">
                                <div class="rounded-2xl border border-slate-200 bg-white px-4 py-3">
                                    ...
                                    <div class="mt-1 font-medium text-slate-900">
                                        <?= htmlspecialchars(trim((string) (($configuracaoLoja['nome_loja'] ?? '') . ((string) ($configuracaoLoja['numero_loja'] ?? '') !== '' ? ' nÂº ' . $configuracaoLoja['numero_loja'] : '')))) ?>
                                    </div>
                                </div>
                                <div class="rounded-2xl border border-slate-200 bg-white px-4 py-3">
                                    <div class="text-xs uppercase tracking-[0.18em] text-slate-500">Ãgape</div>
                                ...
                                <div class="rounded-2xl border border-slate-200 bg-white px-4 py-3">
                                    <div class="text-xs uppercase tracking-[0.18em] text-slate-500">Confirmados com Ã¡gape</div>
                                    <div class="mt-1 text-2xl font-semibold text-slate-900"><?= count($participantesAgape) ?></div>
                                </div>
                                <div class="rounded-2xl border border-slate-200 bg-white px-4 py-3">
                                    <div class="text-xs uppercase tracking-[0.18em] text-slate-500">Estimativa de arrecadaÃ§Ã£o</div>
                            ...
                            <?php if (empty($proximaSessao['reflete_financeiro_oficial'])): ?>
                                <div class="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                                    Esta sessÃ£o nÃ£o gera reflexo automÃ¡tico no financeiro oficial da Loja.
                                </div>
                            <?php endif; ?>
                        </div>
                    <?php else: ?>
                        <div class="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">Nenhuma sessÃ£o futura cadastrada.</div>
                    <?php endif; ?>
                </article>

                <article class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    <h2 class="text-2xl font-semibold text-slate-900">Participantes do Ã¡gape</h2>
                    <p class="mt-2 text-sm text-slate-600">Base operacional para conferÃªncia financeira da sessÃ£o, separando quem confirmou participaÃ§Ã£o no Ã¡gape.</p>
                    <div class="mt-4 space-y-3">
                        <?php if ($participantesAgape !== []): ?>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">Ainda nÃ£o hÃ¡ confirmaÃ§Ãµes com Ã¡gape para a prÃ³xima sessÃ£o.</div>
                        <?php endif; ?>
                    </div>
                </article>
            </section>
            ...
            <aside class="space-y-6">
                        ...
                        <?php if ($sessoesFinanceiras !== []): ?>
                            <?php foreach ($sessoesFinanceiras as $sessao): ?>
                                <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                                    <div class="font-medium text-slate-900"><?= htmlspecialchars($sessao['titulo'] ?: ($sessao['descricao_tipo'] ?: 'SessÃ£o')) ?></div>
                                    ...
                                    <div class="mt-3 grid gap-2 text-sm text-slate-700">
                                        ...
                                        <div>Ãgape: <?= htmlspecialchars((string) ($sessao['descricao_agape'] ?? '-')) ?></div>
                                        <div>Modelo financeiro: <?= htmlspecialchars((string) ($sessao['descricao_modelo_financeiro_agape'] ?? '-')) ?></div>
                                        <div>Confirmados com Ã¡gape: <?= (int) ($sessao['total_agape'] ?? 0) ?></div>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">Nenhuma sessÃ£o futura disponÃ­vel.</div>
                        <?php endif; ?>
                    </div>
                </article>
                ...
                <article class="rounded-3xl border border-slate-200 bg-[linear-gradient(180deg,#fff,#f4efe3)] p-6 shadow-sm">
                    ...
                    <div class="mt-4 space-y-3 text-sm text-slate-700">
                        <div class="rounded-2xl border border-slate-200 bg-white px-4 py-3">O SecretÃ¡rio publica a sessÃ£o e define o modelo financeiro do Ã¡gape (oficial, particular ou misto).</div>
                        <div class="rounded-2xl border border-slate-200 bg-white px-4 py-3">Os membros confirmam com ou sem Ã¡gape.</div>
                        ...
                        <div class="rounded-2xl border border-slate-200 bg-white px-4 py-3">O lanÃ§amento financeiro detalhado continua no livro-caixa, sem perder a origem operacional da sessÃ£o.</div>
                    </div>
                </article>
            </aside>
        </div>
