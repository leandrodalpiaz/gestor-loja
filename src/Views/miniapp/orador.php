...
<div class="mx-auto max-w-lg space-y-4">
    ...
    <div id="conteudo" class="hidden space-y-4">
        <div class="card rounded-2xl p-4 space-y-3">
            <div class="text-sm font-semibold">Sessao em foco</div>
        ...
        <div class="card rounded-2xl p-4">
            <div class="text-sm font-semibold">Cargos da sessao</div>
            <div id="lista-cargos" class="mt-3 space-y-2 text-sm"></div>
        </div>

        <div class="card rounded-2xl p-4">
            <div class="text-sm font-semibold">Eventos registrados</div>
...
function render() {
    ...
    document.getElementById('sessao-titulo').textContent = sessao.titulo || `${sessao.tipo_sessao || 'Sessao'} - ${sessao.grau_sessao || ''}`;
    ...
    renderLista('lista-visitantes', dashboard.visitantes, 'Nenhum visitante resumido nesta sessao.', item => `
        <div class="font-medium">${esc(item.nome || 'Visitante')}</div>
        <div class="mt-1 text-xs text-gray-500">${esc(item.linha_resumida || '')}</div>
    `);

    renderLista('lista-cargos', dashboard.cargos_sessao, 'Sem composicao de cargos registrada.', item => `
    ...
    renderLista('lista-eventos', dashboard.eventos_sessao, 'Nenhum evento registrado nesta sessao.', item => `
        <div class="flex items-center justify-between gap-2">
            <div class="font-medium">${esc(item.titulo || 'Evento')}</div>
