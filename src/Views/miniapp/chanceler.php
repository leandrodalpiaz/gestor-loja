...
function renderPresencas() {
    ...
    if (presencas.length === 0) {
        root.innerHTML = '<div class="rounded-xl border border-dashed border-gray-300 px-3 py-3 text-gray-500">Nenhuma nominata prevista para esta sessao.</div>';
        return;
    }
...
function render() {
    ...
    if (sessao) {
        const params = new URLSearchParams({
            ...
            tipo_sessao: sessao.tipo_sessao || 'Ordinaria',
            grau_sessao: sessao.grau_sessao || 'Mestre Macom',
    ...
    renderPresencas();
    renderLista('lista-confirmados', dashboard.confirmados, 'Sem confirmados nesta sessao.', item => ({
        nome: item.nome,
        linha: item.participara_agape ? 'Confirmado com agape' : 'Confirmado sem agape'
    }));
    renderLista('lista-visitantes', dashboard.visitantes, 'Sem visitantes resumidos nesta sessao.', item => ({
        nome: item.nome,
