...
<div class="mx-auto max-w-lg space-y-4">
    <div>
        ...
        <p class="mt-1 text-sm text-gray-500">Sessoes, confirmados, agape, trabalhos, balaustres e relatorio.</p>
    </div>
    ...
    <div id="conteudo" class="hidden space-y-4">
        <div class="card rounded-2xl p-4">
            <div class="text-xs uppercase tracking-wide text-gray-400">Cadastros</div>
            <div class="mt-3 grid grid-cols-2 gap-3 text-sm">
                ...
                <div class="rounded-xl bg-white/70 p-3"><div class="text-gray-500">Ativos</div><div id="cad-ativos" class="mt-1 text-lg font-semibold"></div></div>
        ...
        <div class="card rounded-2xl p-4">
            <div class="flex items-center justify-between gap-3">
                ...
                <button id="toggle-form" type="button" class="rounded-xl bg-slate-900 px-3 py-2 text-xs font-medium text-white">Nova sessao</button>
            </div>
            <div class="mt-4">
            ...
            <div class="mt-4 grid grid-cols-3 gap-3 text-sm">
                ...
                <div class="rounded-xl bg-white/70 p-3"><div class="text-gray-500">Agape</div><div id="sessao-agape" class="mt-1 text-lg font-semibold"></div></div>
            </div>
        ...
        <form id="form-sessao" class="card hidden rounded-2xl p-4 space-y-3">
            <div class="flex items-center justify-between gap-3">
                <div><div class="text-xs uppercase tracking-wide text-gray-400">Sessao simplificada</div><div id="form-titulo" class="mt-1 text-base font-semibold">Nova sessao</div></div>
                <button id="cancel-form" type="button" class="text-sm text-gray-500">Fechar</button>
            </div>
            <input type="hidden" id="form-sessao-id">
            <div><label class="mb-1 block text-sm font-medium text-gray-700">Titulo</label><input id="titulo" type="text" class="w-full rounded-lg border px-3 py-2 text-sm" required></div>
            <div><label class="mb-1 block text-sm font-medium text-gray-700">Data e hora de inicio</label><input id="data_hora_inicio" type="datetime-local" class="w-full rounded-lg border px-3 py-2 text-sm" required></div>
            ...
            <div><label class="mb-1 block text-sm font-medium text-gray-700">Tipo principal</label><select id="tipo_sessao_principal" class="w-full rounded-lg border px-3 py-2 text-sm"><option value="economica">Economica</option><option value="magna">Magna</option><option value="outra">Outra</option></select></div>
            <div><label class="mb-1 block text-sm font-medium text-gray-700">Subtipo</label><select id="tipo_sessao_subtipo" class="w-full rounded-lg border px-3 py-2 text-sm"><option value="economica_1">Economica de 1 Grau</option><option value="economica_2">Economica de 2 Grau</option><option value="economica_3">Economica de 3 Grau</option><option value="magna_iniciacao">Magna de Iniciacao</option><option value="magna_elevacao">Magna de Elevacao</option><option value="magna_exaltacao">Magna de Exaltacao</option><option value="magna_instalacao">Magna de Instalacao</option><option value="outra">Outra</option></select></div>
            <div><label class="mb-1 block text-sm font-medium text-gray-700">Agape</label><select id="agape_modalidade" class="w-full rounded-lg border px-3 py-2 text-sm"><option value="nao_havera">Nao havera</option><option value="gratuito">Gratuito</option><option value="pago">Pago</option></select></div>
            ...
            <button type="submit" class="w-full rounded-xl bg-slate-900 px-4 py-3 text-sm font-medium text-white">Salvar sessao</button>
        </form>
        ...
        <div class="grid gap-4 md:grid-cols-2">
            ...
            <div class="card rounded-2xl p-4"><div class="text-sm font-semibold">Agape</div><div id="lista-agape" class="mt-3 space-y-2 text-sm"></div></div>
        </div>

        <div class="card rounded-2xl p-4"><div class="text-sm font-semibold">Historico recente</div><div id="lista-historico" class="mt-3 space-y-2 text-sm"></div></div>

        <div class="grid gap-4 md:grid-cols-2">
            <div class="card rounded-2xl p-4">
                <div class="text-sm font-semibold">Trabalhos recentes</div>
                <div class="mt-3 space-y-2">
                    <input id="trabalho-titulo" type="text" class="w-full rounded-lg border px-3 py-2 text-sm" placeholder="Titulo do trabalho">
                    <select id="trabalho-tipo" class="w-full rounded-lg border px-3 py-2 text-sm"><option value="peca_arquitetura">Peca de arquitetura</option><option value="instrucao">Instrucao</option><option value="prancha">Prancha</option></select>
            ...
            <div class="card rounded-2xl p-4">
                ...
                <div class="mt-3 space-y-2">
                    <input id="balaustre-numero" type="text" class="w-full rounded-lg border px-3 py-2 text-sm" placeholder="Numero do balaustre">
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        ...
                        <button id="btn-balaustre-abrir" class="rounded-xl bg-emerald-700 px-3 py-2 text-xs font-medium text-white">Abrir votacao</button>
        ...
        <div class="card rounded-2xl p-4">
            <div class="text-sm font-semibold">Relatorio anual</div>
            <div class="mt-3 grid grid-cols-2 gap-3 text-sm">
                <div class="rounded-xl bg-white/70 p-3"><div class="text-gray-500">Sessoes</div><div id="rel-sessoes" class="mt-1 text-lg font-semibold"></div></div>
...
function preencherFormulario(sessao = null) {
    ...
    document.getElementById('form-titulo').textContent = sessao ? 'Editar sessao' : 'Nova sessao';
...
async function acaoSessao(url) {
    ...
    try {
        ...
        tg.showAlert('Operacao realizada com sucesso.');
...
document.getElementById('form-sessao').addEventListener('submit', async (event) => {
    ...
    try {
        ...
        tg.showAlert('Sessao salva com sucesso.');
...
document.getElementById('btn-balaustre-abrir').addEventListener('click', () => acaoMiniapp('/api/miniapp/secretaria/balaustre/abrir-votacao', { balaustre_id: dashboard?.balaustre_foco?.id || 0 }, 'Votacao aberta com sucesso.'));
document.getElementById('btn-balaustre-encerrar').addEventListener('click', () => acaoMiniapp('/api/miniapp/secretaria/balaustre/encerrar-votacao', { balaustre_id: dashboard?.balaustre_foco?.id || 0 }, 'Votacao encerrada com sucesso.'));
