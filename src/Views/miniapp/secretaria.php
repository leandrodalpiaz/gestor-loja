...
<div class="mx-auto max-w-lg space-y-4">
    <div>
        ...
        <p class="mt-1 text-sm text-gray-500">Sessões, confirmados, ágape e visão rápida da Secretaria.</p>
    </div>
    ...
    <div id="conteudo" class="hidden space-y-4">
        <div class="card rounded-2xl p-4">
            ...
            <div class="mt-3 grid grid-cols-2 gap-3 text-sm">
                <div class="rounded-xl bg-white/70 p-3">
                    ...
                    <div id="cad-total" class="mt-1 text-lg font-semibold"></div>
                </div>
                <div class="rounded-xl bg-white/70 p-3">
                    <div class="text-gray-500">Alertas</div>
                    <div id="cad-alerta" class="mt-1 text-lg font-semibold"></div>
                </div>
                <div class="rounded-xl bg-white/70 p-3">
                    <div class="text-gray-500">Ativos</div>
                    <div id="cad-ativos" class="mt-1 text-lg font-semibold"></div>
                </div>
                <div class="rounded-xl bg-white/70 p-3">
                    <div class="text-gray-500">Com bot</div>
                    <div id="cad-bot" class="mt-1 text-lg font-semibold"></div>
                </div>
            </div>
        </div>
        ...
        <div class="card rounded-2xl p-4">
            ...
            <div class="mt-4 grid grid-cols-3 gap-3 text-sm">
                <div class="rounded-xl bg-white/70 p-3">
                    ...
                    <div id="sessao-confirmados" class="mt-1 text-lg font-semibold"></div>
                </div>
                <div class="rounded-xl bg-white/70 p-3">
                    <div class="text-gray-500">Ausentes</div>
                    <div id="sessao-ausentes" class="mt-1 text-lg font-semibold"></div>
                </div>
                <div class="rounded-xl bg-white/70 p-3">
                    <div class="text-gray-500">Ágape</div>
                    <div id="sessao-agape" class="mt-1 text-lg font-semibold"></div>
                </div>
            </div>
        ...
        <form id="form-sessao" class="card hidden rounded-2xl p-4 space-y-3">
            <div class="flex items-center justify-between gap-3">
                <div>
                    <div class="text-xs uppercase tracking-wide text-gray-400">Sessão simplificada</div>
                    <div id="form-titulo" class="mt-1 text-base font-semibold">Nova sessão</div>
                </div>
            ...
            <div>
                ...
                <input id="titulo" type="text" class="w-full rounded-lg border px-3 py-2 text-sm" required>
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium text-gray-700">Data e hora de início</label>
                <input id="data_hora_inicio" type="datetime-local" class="w-full rounded-lg border px-3 py-2 text-sm" required>
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium text-gray-700">Grau</label>
                <select id="grau_sessao" class="w-full rounded-lg border px-3 py-2 text-sm">
            ...
            <div>
                <label class="mb-1 block text-sm font-medium text-gray-700">Tipo principal</label>
                <select id="tipo_sessao_principal" class="w-full rounded-lg border px-3 py-2 text-sm">
            ...
            <div>
                <label class="mb-1 block text-sm font-medium text-gray-700">Subtipo</label>
                <select id="tipo_sessao_subtipo" class="w-full rounded-lg border px-3 py-2 text-sm">
                    <option value="economica_1">Econômica de 1 Grau</option>
                    <option value="economica_2">Econômica de 2 Grau</option>
                    <option value="economica_3">Econômica de 3 Grau</option>
                    <option value="magna_iniciacao">Magna de Iniciação</option>
                    <option value="magna_elevacao">Magna de Elevação</option>
                    <option value="magna_exaltacao">Magna de Exaltação</option>
                    <option value="magna_instalacao">Magna de Instalação</option>
                    <option value="outra">Outra</option>
                </select>
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium text-gray-700">Ágape</label>
                <select id="agape_modalidade" class="w-full rounded-lg border px-3 py-2 text-sm">
                    <option value="nao_havera">Não haverá</option>
                    <option value="gratuito">Gratuito</option>
                    <option value="pago">Pago</option>
                </select>
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium text-gray-700">Ordem do dia</label>
                <textarea id="ordem_dia" rows="3" class="w-full rounded-lg border px-3 py-2 text-sm"></textarea>
            </div>
        ...
        <div class="grid gap-4 md:grid-cols-2">
            ...
            <div class="card rounded-2xl p-4">
                <div class="text-sm font-semibold">Ágape</div>
                <div id="lista-agape" class="mt-3 space-y-2 text-sm"></div>
            </div>
        </div>

        <div class="card rounded-2xl p-4">
            <div class="text-sm font-semibold">Histórico recente</div>
        ...
        <div class="card rounded-2xl p-4">
            ...
            <div class="mt-3 grid grid-cols-2 gap-3 text-sm">
                <div class="rounded-xl bg-white/70 p-3">
                    ...
                    <div id="rel-sessoes" class="mt-1 text-lg font-semibold"></div>
                </div>
                <div class="rounded-xl bg-white/70 p-3">
                    <div class="text-gray-500">Visitantes</div>
                    <div id="rel-visitantes" class="mt-1 text-lg font-semibold"></div>
                </div>
                <div class="rounded-xl bg-white/70 p-3">
                    <div class="text-gray-500">Visitas externas</div>
                    <div id="rel-visitas" class="mt-1 text-lg font-semibold"></div>
                </div>
                <div class="rounded-xl bg-white/70 p-3">
                    <div class="text-gray-500">Congressos/Palestras</div>
                    <div id="rel-eventos" class="mt-1 text-lg font-semibold"></div>
                </div>
            </div>
        </div>
    </div>
...
async function api(url, options = {}) {
    ...
    if (!json.ok) {
        throw new Error(json.erro || 'Falha na operação.');
    }
    return json;
}
...
function renderLista(id, itens, vazio) {
    ...
    itens.forEach(item => {
        ...
        div.innerHTML = `<div class="font-medium">${esc(item.nome || item.acao || 'Item')}</div>
            ${item.cim ? `<div class="text-xs text-gray-500">CIM ${esc(item.cim)}</div>` : ''}
            ${item.observacao ? `<div class="text-xs text-gray-500 mt-1">${esc(item.observacao)}</div>` : ''}
            ${item.autor_nome ? `<div class="text-xs text-gray-500 mt-1">${esc(item.autor_nome)} · ${esc(item.created_at || '')}</div>` : ''}`;
...
function render() {
    ...
    renderLista('lista-trabalhos', (dashboard.trabalhos_recentes || []).map(item => ({
        nome: item.titulo || 'Trabalho',
        observacao: `${item.sessao_titulo || 'Sessão'} · ${item.status_envio_potencia || 'pendente'}`
    })), 'Sem trabalhos recentes.');
    renderLista('lista-balaustres', (dashboard.balaustres_recentes || []).map(item => ({
        nome: item.numero_balaustre || 'Balaustre sem número',
        observacao: `${item.sessao_titulo || 'Sessão'} · ${item.status || ''}`
    })), 'Sem balaustres recentes.');
