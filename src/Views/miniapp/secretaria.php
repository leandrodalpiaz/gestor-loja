...
<div class="mx-auto max-w-lg space-y-4">
    <div>
        ...
        <p class="mt-1 text-sm text-gray-500">SessÃµes, confirmados, Ã¡gape, trabalhos, balaustres e relatÃ³rio.</p>
    </div>
    ...
    <div id="conteudo" class="hidden space-y-4">
        ...
        <div class="card rounded-2xl p-4">
            <div class="flex items-center justify-between gap-3">
                ...
                <button id="toggle-form" type="button" class="rounded-xl bg-slate-900 px-3 py-2 text-xs font-medium text-white">Nova sessÃ£o</button>
            </div>
            <div class="mt-4">
        ...
        <form id="form-sessao" class="card hidden rounded-2xl p-4 space-y-3">
            <div class="flex items-center justify-between gap-3">
                <div><div class="text-xs uppercase tracking-wide text-gray-400">SessÃ£o simplificada</div><div id="form-titulo" class="mt-1 text-base font-semibold">Nova sessÃ£o</div></div>
            ...
            <div><label class="mb-1 block text-sm font-medium text-gray-700">Subtipo</label><select id="tipo_sessao_subtipo" class="w-full rounded-lg border px-3 py-2 text-sm"><option value="economica_1">EconÃ´mica de 1 Grau</option><option value="economica_2">EconÃ´mica de 2 Grau</option><option value="economica_3">EconÃ´mica de 3 Grau</option><option value="magna_iniciacao">Magna de IniciaÃ§Ã£o</option><option value="magna_elevacao">Magna de ElevaÃ§Ã£o</option><option value="magna_exaltacao">Magna de ExaltaÃ§Ã£o</option><option value="magna_instalacao">Magna de InstalaÃ§Ã£o</option><option value="outra">Outra</option></select></div>
        ...
        <div class="grid gap-4 md:grid-cols-2">
            ...
            <div class="card rounded-2xl p-4">
                ...
                <div class="mt-3 space-y-2">
                    <input id="balaustre-numero" type="text" class="w-full rounded-lg border px-3 py-2 text-sm" placeholder="NÃºmero do balaustre">
...
function render() {
    ...
    renderLista('lista-balaustres', (dashboard.balaustres_recentes || []).map(item => ({ nome: item.numero_balaustre || 'Balaustre sem nÃºmero', observacao: `${item.sessao_titulo || 'SessÃ£o'} Â· ${item.status || ''}` })), 'Sem balaustres recentes.');
...
document.getElementById('btn-balaustre-abrir').addEventListener('click', () => acaoMiniapp('/api/miniapp/secretaria/balaustre/abrir-votacao', { balaustre_id: dashboard?.balaustre_foco?.id || 0 }, 'VotaÃ§Ã£o aberta com sucesso.'));
document.getElementById('btn-balaustre-encerrar').addEventListener('click', () => acaoMiniapp('/api/miniapp/secretaria/balaustre/encerrar-votacao', { balaustre_id: dashboard?.balaustre_foco?.id || 0 }, 'VotaÃ§Ã£o encerrada com sucesso.'));
