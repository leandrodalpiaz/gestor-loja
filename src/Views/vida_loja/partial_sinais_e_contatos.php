...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 space-y-8">
        ...
        <!-- SINAIS DE ATENÇÃO PREVENTIVA POR AUSÊNCIA -->
        <div class="card depth-1">
            <div class="card-header border-b border-white/5 p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h2 class="card-title text-white">Atenção Preventiva por Ausência</h2>
                    <p class="card-subtitle mt-1">Obreiros com ausências acumuladas nos últimos 60 dias (sinais gerados a partir do histórico de presenças).</p>
                </div>
            ...
            <div class="card-body p-6">
                ...
                <div class="hidden md:block overflow-x-auto">
                    <table class="table-base w-full">
                        ...
                        <tbody class="divide-y divide-white/5 text-sm">
                            <?php if (empty($sinais)): ?>
                                <tr>
                                    <td colspan="6" class="py-8 text-center text-slate-400">Nenhum sinal de atenção preventiva ativo na Loja.</td>
                                </tr>
                            <?php else: ?>
                ...
                <div class="block md:hidden space-y-4">
                    <?php if (empty($sinais)): ?>
                        <p class="text-center text-slate-400 py-6 text-sm">Nenhum sinal de cuidado ativo.</p>
                    <?php else: ?>
                        <?php foreach ($sinais as $si): ?>
                    ...
                    <form method="POST" action="/vida-loja/contato/salvar" class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="nivel_sigilo" id="nivel_sigilo" class="form-select w-full" onchange="document.getElementById('sigiloso_form_block').style.display = (this.value !== 'administrativo') ? 'block' : 'none';">
                                ...
                                <option value="administrativo">Administrativo (Livre a todos com acesso ao Painel)</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="text-[9px] text-slate-400 mt-1">Planejamento de nova ação de acompanhamento.</p>
                        </div>
        ...
        <div class="card depth-1 p-6">
            ...
            <div class="card-body text-xs text-slate-400 space-y-3 leading-relaxed">
                <div class="flex gap-2">
                    ...
                    <p><strong class="text-white">Atenção Prioritária:</strong> Gerado para frequência individual igual ou inferior a 50% nas últimas sessões realizadas.</p>
                </div>
                <div class="flex gap-2">
                    <span class="inline-flex rounded-full w-2 h-2 mt-1.5 shrink-0 bg-orange-500"></span>
                    <p><strong class="text-white">Atenção Recomendada:</strong> Gerado para frequência individual acumulada entre 51% e 69%.</p>
                </div>
                <div class="flex gap-2">
        ...
        <form method="POST" action="/vida-loja/contato/excluir" class="space-y-4">
            ...
            <div>
                ...
                <textarea name="motivo_exclusao" id="modal-excluir-motivo" rows="3" required class="form-textarea w-full text-xs" 
                          placeholder="Descreva o motivo pelo qual este contato está sendo removido..."></textarea>
            </div>
