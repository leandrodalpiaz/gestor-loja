        ...
        <?php if (!$somenteProprio): ?>
            <section class="mt-6 grid gap-6 xl:grid-cols-2">
                <article class="rounded-3xl border border-white/70 bg-white/90 p-6 shadow-sm">
                    ...
                    <p class="mt-1 text-sm text-slate-600">Registre andamento, recebimento, revisao e devolutiva do Companheiro.</p>
                ...
                <article id="leitura-sugerida" class="rounded-3xl border border-white/70 bg-white/90 p-6 shadow-sm">
                    ...
                    <p class="mt-1 text-sm text-slate-600">Conecte o acompanhamento do Companheiro com a biblioteca da Loja.</p>
        ...
        <section class="mt-6 grid gap-6 xl:grid-cols-2">
            <article class="rounded-3xl border border-white/70 bg-white/90 p-6 shadow-sm">
                <div class="flex items-center justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-600">Cada etapa mostra o estado real da jornada do Companheiro.</p>
                    </div>
                    <div class="w-40">
            ...
            <div class="space-y-6">
                <article id="certificado" class="rounded-3xl border border-white/70 bg-white/90 p-6 shadow-sm">
                    ...
                    <p class="mt-1 text-sm text-slate-600">Solicitacao formal da conclusao da docencia do Companheiro.</p>
                    <div class="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                        ...
                        <?php if (!empty($acompanhamento['certificado_solicitado_em'])): ?>
                            <div class="mt-1 text-sm text-slate-600">Solicitado em <?= htmlspecialchars(date('d/m/Y H:i', strtotime((string) $acompanhamento['certificado_solicitado_em']))) ?></div>
                        <?php endif; ?>
                    </div>
                    <?php if (!$somenteProprio): ?>
                ...
                <article id="exaltacao" class="rounded-3xl border border-white/70 bg-white/90 p-6 shadow-sm">
                    ...
                    <p class="mt-1 text-sm text-slate-600">Encaminhamento formal do Companheiro para apreciacao da exaltação.</p>
                    <div class="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                        <div class="text-xs uppercase tracking-wide text-slate-500">Status</div>
                        ...
                        <?php if (!empty($acompanhamento['exaltacao_recomendada_em'])): ?>
                            <div class="mt-1 text-sm text-slate-600">Recomendada em <?= htmlspecialchars(date('d/m/Y H:i', strtotime((string) $acompanhamento['exaltacao_recomendada_em']))) ?></div>
                        <?php endif; ?>
                    </div>
                    <?php if (!$somenteProprio): ?>
                ...
                <article class="rounded-3xl border border-white/70 bg-white/90 p-6 shadow-sm">
                    ...
                    <p class="mt-1 text-sm text-slate-600">Linha consolidada da trilha, leitura, certificado e exaltacao.</p>
                    <div class="mt-4 space-y-3">
                        <?php foreach ($historicoFormativo as $evento): ?>
                            <div class="rounded-2xl border border-slate-200 px-4 py-3">
                                <div class="text-xs uppercase tracking-wide text-slate-500"><?= htmlspecialchars((string) ($evento['tipo'] ?? 'evento')) ?></div>
                                <div class="mt-1 text-sm font-medium text-slate-800"><?= htmlspecialchars((string) ($evento['titulo'] ?? 'Marco formativo')) ?></div>
                                <div class="mt-1 text-xs text-slate-500"><?= !empty($evento['momento']) ? htmlspecialchars(date('d/m/Y H:i', strtotime((string) $evento['momento']))) : '-' ?></div>
                                <?php if (!empty($evento['descricao'])): ?>
                                    <div class="mt-2 text-sm text-slate-600"><?= htmlspecialchars((string) $evento['descricao']) ?></div>
                                <?php endif; ?>
                            </div>
                        <?php endforeach; ?>
                        <?php if ($historicoFormativo === []): ?>
                            <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-500">Ainda nao ha marcos registrados para este Companheiro.</div>
                        <?php endif; ?>
                    </div>
                </article>
            </div>
