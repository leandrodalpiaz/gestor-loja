...
for ($mes = 1; $mes <= 12; $mes++) {
    ...
    $mesesTesourarias[$chaveMes] = [
        'chave' => $chaveMes,
        'rotulo' => ($nomesMeses[$mes] ?? 'MÃªs') . ' ' . $anoPainel,
        ...
        'itens' => [[
            'item_base' => true,
            'obrigacao_titulo' => 'Contribuicao mensal da Loja',
            ...
            'descricao_status' => $mensalidadePaga
                ? 'Contribuicao mensal considerada paga no ajuste inicial de 2026.'
                : 'Contribuicao mensal prevista para o mÃªs.',
            'em_atraso' => false,
            'pago_em' => null,
        ]],
    ];
    ...
    if (isset($bibliotecaPorMes[$mes])) {
        ...
        $mesesTesourarias[$chaveMes]['itens'][] = [
            'item_base' => true,
            'obrigacao_titulo' => 'ContribuiÃ§Ã£o Biblioteca',
            ...
            'descricao_status' => $bibliotecaPaga
                ? 'ContribuiÃ§Ã£o considerada paga no ajuste inicial de 2026.'
                : 'ContribuiÃ§Ã£o da biblioteca prevista para este mÃªs.',
            'em_atraso' => false,
            'pago_em' => null,
        ];
    }
...
$resumoPagoTexto = count($parcelasPagas) > 0
    ...
    : 'Nenhum pagamento confirmado atÃ© aqui.';
...
<div class="mx-auto max-w-7xl px-4 py-6">
            ...
            <div class="mt-3 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div class="max-w-3xl">
                    ...
                    <p class="mt-2 text-sm leading-6 text-slate-300">
                        Agora o painel destaca competÃªncia por competÃªncia, mostrando o que jÃ¡ foi pago, o que ainda estÃ¡ previsto e os prÃ³ximos compromissos da Loja.
                    </p>
                </div>
                <div class="rounded-3xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-200">
                    ...
                    <div class="mt-1 text-slate-300">Janeiro, fevereiro e marÃ§o de 2026 aparecem como quitados no ajuste inicial do sistema.</div>
                </div>
            </div>
            <div class="mt-4 flex flex-wrap items-center gap-3">
                <div class="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-200">
                    PIX <?php echo htmlspecialchars($pixTipo); ?>: <?php echo htmlspecialchars($pixValor ?: 'NÃ£o informado'); ?>
                </div>
    ...
    $dashboard = [
        ...
        'subtitle' => 'Consumo pessoal: financeiro, biblioteca e emprÃ©stimos.',
        'meta' => ['Perfil: obreiro', 'Sem operaÃ§Ã£o administrativa'],
        'actions' => [
            ['label' => 'Ver obrigaÃ§Ãµes', 'href' => '/financeiro/minhas-obrigacoes'],
            ['label' => 'Solicitar item', 'href' => '/biblioteca'],
            ['label' => 'Ver meus emprÃ©stimos', 'href' => '/biblioteca/meus-emprestimos'],
        ],
        'blocks' => [
            ['title' => 'Minhas obrigaÃ§Ãµes', 'subtitle' => 'Resumo financeiro pessoal.', 'span' => 'half', 'metrics' => [
                ['label' => 'Total pago', 'value' => $formatCurrency($totalPagoExibicao)],
                ['label' => 'Aguardando confirmaÃ§Ã£o', 'value' => (string) count($parcelasAguardandoConfirmacao)],
                ['label' => 'Atrasadas', 'value' => (string) count($parcelasAtrasadas)],
            ], 'list' => array_map(static fn (array $p): array => ['item' => (string) ($p['obrigacao_titulo'] ?? 'ObrigaÃ§Ã£o'), 'meta' => (string) ($p['competencia_label'] ?? '-'), 'status' => (string) ($p['status_exibicao'] ?? 'programada')], array_slice($parcelasAguardandoConfirmacao ?: $parcelasProgramadas, 0, 4))],
            ['title' => 'Biblioteca e interaÃ§Ãµes', 'subtitle' => 'Uso pessoal do acervo.', 'span' => 'half', 'metrics' => [
                ['label' => 'Pagas recentes', 'value' => (string) count($pagasRecentes)],
                ['label' => 'Proximos compromissos', 'value' => (string) count($proximosCompromissos)],
            ], 'list' => [['item' => 'Biblioteca', 'meta' => 'Catalogo e solicitacoes', 'status' => 'Regular'], ['item' => 'Meus emprestimos', 'meta' => 'Acompanhamento pessoal', 'status' => 'Regular']]],
        ],
        'alerts' => [['title' => 'Compromissos financeiros', 'text' => 'Contribuicoes e confirmaÃ§Ãµes devem seguir os canais oficiais da tesouraria.', 'tone' => (count($parcelasAtrasadas) > 0 ? 'danger' : 'warning')]],
        'activity' => array_map(static fn (array $p): array => ['item' => (string) ($p['obrigacao_titulo'] ?? 'ObrigaÃ§Ã£o'), 'meta' => 'Pago em ' . (string) ($p['pago_em'] ?? '-')] , array_slice($pagasRecentes, 0, 4)),
        'links' => [['label' => 'Biblioteca', 'href' => '/biblioteca'], ['label' => 'Meus emprÃ©stimos', 'href' => '/biblioteca/meus-emprestimos']],
    ];
    $dashboardRenderers = [
    ...
    <section class="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="rounded-[1.75rem] border border-amber-100 bg-amber-50 p-5 shadow-sm">
            <div class="text-xs uppercase tracking-[0.18em] text-amber-500">PrÃ³xima obrigaÃ§Ã£o</div>
            ...
            <div class="mt-2 text-sm text-amber-700">
                <?php if ($proximaObrigacao): ?>
                    <?php echo htmlspecialchars((string) ($proximaObrigacao['obrigacao_titulo'] ?? 'ObrigaÃ§Ã£o')); ?> de <?php echo htmlspecialchars((string) ($proximaObrigacao['competencia_label'] ?? '')); ?>.
                <?php else: ?>
                    Nenhuma obrigaÃ§Ã£o pendente no momento.
                <?php endif; ?>
            </div>
        </div>
        <?php if (!empty($parcelasAguardandoConfirmacao) || !empty($parcelasAtrasadas)): ?>
            <div class="rounded-[1.75rem] border border-rose-100 bg-rose-50 p-5 shadow-sm">
                ...
                <div class="mt-2 text-sm text-rose-700">Contribuicoes que exigem confirmacao ou tratativa com a Tesouraria.</div>
            </div>
        <?php else: ?>
        ...
        <div class="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm">
            <div class="text-xs uppercase tracking-[0.18em] text-slate-400">PrÃ³ximo marco</div>
            <div class="mt-2 text-2xl font-semibold text-slate-900">
                <?php echo $proximaObrigacao && !empty($proximaObrigacao['vencimento']) ? htmlspecialchars(date('d/m/Y', strtotime((string) $proximaObrigacao['vencimento']))) : 'Sem previsÃ£o'; ?>
            </div>
            <div class="mt-2 text-sm text-slate-700">Contribuicoes mensais seguem orientadas para o dia 10. ObrigaÃ§Ãµes futuras ficam apenas programadas atÃ© se aproximarem do pagamento.</div>
        </div>
        <?php if ($joiaResumo !== null): ?>
            <div class="rounded-[1.75rem] border border-violet-100 bg-violet-50 p-5 shadow-sm sm:col-span-2 xl:col-span-4">
                ...
                <div class="mt-2 text-xs text-violet-600">ReferÃªncia do salÃ¡rio mÃ­nimo nacional em 2026: R$ 1.621,00.</div>
            </div>
        <?php endif; ?>
    </section>
    ...
    <section class="mt-6 grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
            ...
            <section class="rounded-[2rem] border border-stone-200 bg-white p-6 shadow-sm">
                <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        ...
                        <h2 class="mt-1 text-2xl font-semibold">MÃªs a mÃªs da Loja</h2>
                    </div>
                    <div class="text-sm text-slate-700">Visualmente aparente: total pago, total previsto e lista de obrigaÃ§Ãµes dentro de cada competÃªncia.</div>
                </div>
                        ...
                        <article class="rounded-[1.75rem] border p-5 <?php echo $cardMesClass; ?>">
                            <div class="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                                <div>
                                    ...
                                    <p class="mt-2 text-sm leading-6 text-slate-700">Resumo consolidado do mÃªs para vocÃª enxergar rapidamente o que jÃ¡ entrou e o que ainda depende de pagamento.</p>
                                ...
                                <div class="grid gap-2 rounded-[1.5rem] bg-white p-4 text-sm text-slate-700 shadow-sm sm:grid-cols-3 xl:min-w-[380px]">
                                    ...
                                    <div>
                                        <div class="text-xs uppercase tracking-[0.16em] text-slate-400">Aguardando confirmaÃ§Ã£o</div>
                            ...
                            <div class="mt-5 grid gap-3 lg:grid-cols-2">
                                <?php if ($mesTesouraria['itens'] === []): ?>
                                    <div class="lg:col-span-2 rounded-[1.35rem] border border-dashed border-stone-300 bg-white px-4 py-5 text-sm text-slate-700">
                                        Nenhuma obrigaÃ§Ã£o lanÃ§ada neste mÃªs ainda. Quando houver mensalidade, biblioteca, joia ou outra cobranÃ§a, ela aparecerÃ¡ aqui.
                                    </div>
                                <?php endif; ?>
        ...
        <div class="space-y-6">
            <section class="rounded-[2rem] border border-stone-200 bg-white p-6 shadow-sm">
                <p class="text-xs uppercase tracking-[0.22em] text-slate-400">Contribuicoes</p>
                    <h2 class="mt-1 text-2xl font-semibold">HistÃ³rico recente</h2>
                <div class="mt-4 space-y-3">
                    <?php if ($pagasRecentes === []): ?>
                        <div class="rounded-[1.5rem] border border-dashed border-stone-300 bg-stone-50 p-5 text-sm text-slate-700">Ainda nÃ£o hÃ¡ compromissos reconhecidos como pagos.</div>
                    <?php endif; ?>
            ...
            <section class="rounded-[2rem] border border-stone-200 bg-white p-6 shadow-sm">
                ...
                <div class="mt-4 space-y-3">
                    <?php if ($proximosCompromissos === []): ?>
                        <div class="rounded-[1.5rem] border border-dashed border-stone-300 bg-stone-50 p-5 text-sm text-slate-700">Nenhuma obrigaÃ§Ã£o futura programada no momento.</div>
                    <?php endif; ?>
            ...
            <section class="rounded-[2rem] border border-rose-100 bg-rose-50 p-6 shadow-sm">
                ...
                <h2 class="mt-1 text-2xl font-semibold text-rose-900">PendÃªncias que pedem ajuste</h2>
                <div class="mt-4 space-y-3">
