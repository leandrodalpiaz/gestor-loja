...
$rotulosBloco = [
    ...
    'despesas_potencia' => 'SaÃ­das com a PotÃªncia',
    'despesas_administrativas' => 'SaÃ­das administrativas',
    'despesas_bancarias' => 'SaÃ­das bancÃ¡rias',
    'despesas_ritualisticas' => 'SaÃ­das ritualÃ­sticas',
...
$appShellTitle = 'RelatÃ³rio Financeiro da GestÃ£o';
$appShellDescription = 'ConsolidaÃ§Ã£o por perÃ­odo para prestaÃ§Ã£o de contas e anÃ¡lise administrativa.';
...
<div class="card mb-6">
    <form method="GET" action="/tesouraria/relatorio-gestao" class="card-body">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            <div>
                <label for="gestao_id" class="form-label">GestÃ£o</label>
            ...
            <button type="submit" class="btn btn-primary w-full md:w-auto">Atualizar RelatÃ³rio</button>
        </div>
    </form>
</div>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    <div class="card-metric"><p class="card-metric-label">Total de SaÃ­das</p><p class="card-metric-value text-red-600 dark:text-red-400"><?= $formatCurrency($relatorio['totais']['saidas'] ?? 0) ?></p></div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 space-y-8">
        <div class="card">
            <div class="card-header"><h2 class="card-title">SÃ­ntese por Blocos</h2><p class="card-description">Agrupamentos para o relatÃ³rio da gestÃ£o.</p></div>
                ...
                <?php foreach (($relatorio['blocos'] ?? []) as $bloco => $totais): ?>
                    <div class="list-item-report">
                        ...
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            ...
                            <div class="flex justify-between items-baseline"><span class="text-gray-500">SaÃ­das:</span><strong class="text-red-600 dark:text-red-400"><?= $formatCurrency($totais['saida'] ?? 0) ?></strong></div>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
                ...
                <?php foreach (($relatorio['categorias'] ?? []) as $linha): ?>
                    <div class="list-item-detail">
                        ...
                        <div class="text-right">
                            ...
                            <p class="text-xs text-gray-500"><?= (int) ($linha['quantidade'] ?? 0) ?> lanÃ§amentos</p>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
    ...
    <div class="space-y-8">
        <div class="card">
            ...
            <div class="card-body">
                <div class="grid grid-cols-2 gap-4 mb-6">
                    ...
                    <div class="card-metric-simple text-center"><p class="card-metric-label">SaÃ­das Tronco</p><p class="card-metric-value text-red-600 dark:text-red-400 text-xl"><?= $formatCurrency($relatorio['tronco']['saidas'] ?? 0) ?></p></div>
                </div>
                ...
                <div class="space-y-3">
                    <?php if (empty($relatorio['entidades_auxiliadas'])): ?>
                        <div class="text-center py-4 px-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg"><p class="text-sm text-gray-500">Nenhuma entidade auxiliada no perÃ­odo.</p></div>
                    <?php else: ?>
        ...
        <div class="card">
            <div class="card-header"><h2 class="card-title">LanÃ§amentos Recentes</h2></div>
            <div class="card-body space-y-4">
                <?php foreach (($relatorio['lancamentos'] ?? []) as $lancamento): ?>
                    <div class="list-item-detail">
                        <div>
                            ...
                            <p class="text-xs text-gray-600 mt-1"><?= htmlspecialchars((string) ($lancamento['descricao'] ?? 'Sem descriÃ§Ã£o')) ?></p>
